...
public class StrategyArmoryDispatchTest {
    ...
    public void testGetRandomRuleLockAwardId() {
        for (int i = 0; i < 50; i++) {
            Long randomAwardId = strategyDispatch.getRuleLockAwardIdByRandom(10001L);
    ...
    public void testGetRandomRuleLockLongAwardId() {
        for (int i = 0; i < 50; i++) {
            Long randomAwardId = strategyDispatch.getRuleLockLongAwardIdByRandom(10001L);
            log.atInfo().log("中奖的奖品id : {}", randomAwardId);
        }
    }
    ...
    public void testGetWorstAwardId() {
        Long worstAwardId = strategyDispatch.getWorstAwardId(10001L);
        log.atInfo().log("黑名单用户的奖品id : {}", worstAwardId);
    }

    @Test
    public void testGetGrandAwardIdByRandom() {
        for (int i = 0; i < 100; i++) {
            Long grandAwardId = strategyDispatch.getRuleGrandAwardIdByRandom(10001L);
            log.atInfo().log("大奖池中的奖品id : {}", grandAwardId);
        }
    }
}
